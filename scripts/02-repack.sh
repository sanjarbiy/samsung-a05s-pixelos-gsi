...
BIN="$(find "$HERE/../.tools" -name lpmake -printf '%h\n' 2>/dev/null | head -1)"
...
echo "[*] extract super.img.lz4 + vbmeta.img.lz4 from AP"
tar xf "$AP" -C "$W" super.img.lz4 vbmeta.img.lz4 2>/dev/null || tar xf "$AP" -C "$W" super.img.lz4
...
if [ "$TOTAL" -gt "$GMAX" ]; then
  echo "[!] GSI too big by $((TOTAL-GMAX)) bytes. Shrink system.img:"
  echo "    e2fsck -y -E unshare_blocks $W/parts/system.img; resize2fs -M $W/parts/system.img ; then re-run."
  exit 2
fi
echo "    OK, headroom=$((GMAX-TOTAL))"
